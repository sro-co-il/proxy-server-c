...
char * SroPullouthostname(char *request, SOCKET Socket)
	...
	// because if not, strstr change the destination of the pointer request!
	...
	host = strstr(strcpy(host, request), "\nHost: ") + 7; // cut until after the word "Host: "
	if (strlen(host) == strlen(request))
...
void SroProxy(void * Pid)
	...
	// create buffers to read the request and response
	char request[2048];
	int requestLen;

	// buffer for destination host name	
	char *host;

	// read the request from socket
	requestLen = recv(Socket, request, sizeof(request) - 1, 0);
	// for debugging
	printf("\r\n\r\n\r\n--- new request\r\n");
	
	// while for checking if in the same socket, the browser send additional request.
	// it's happen when the server receive 403, or header "location" etc.
	while (requestLen > 0)
		{
		...
		host = SroPullouthostname(request, Socket);
		...
		// try to receive additional request
		requestLen = recv(Socket, request, sizeof(request) - 1, 0);
...
int main(int argc, char *argv[])
	...
	SOCKADDR_IN SockAddr = SroBind(SockListen, INADDR_ANY, PORT); //inet_addr("127.0.0.1")
	// start Listen
